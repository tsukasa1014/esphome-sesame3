...
- 標準BLE移植のレビューで見つかった問題の修正:
  ...
  - 正常なポーリング終了（`always_connect: false`）では測定値を保持し、その後の切断完了処理でも
    消えないようにした。タイムアウトやプロトコルエラーなどの異常終了では、v0.31.0 と同じく
    値を破棄する。あわせて `unknown_state_timeout` が実際に機能するよう修正した
    （`0s` は猶予なしとして即座に不明へ、正の秒数はその時間の経過後に不明へ）。
  - 復旧の計数を整理した。「リンクが開かない」経路と、親クライアントの切断待ちタイムアウト
    （CLOSEイベントが来ない場合）を、どちらも同じ停滞カウンタへ合流させた。コントローラが
    同期拒否した接続試行も、設定した再試行バックオフを通る。
  ...
  - ドキュメント: サンプルの `external_components` を移植版を含むforkブランチへ変更、再試行の説明を実装どおりに修正、
    履歴の例から解錠などの操作を削除、セキュリティ上の注意にライブラリの平文受理と
    `text_sensor` のVERBOSEログ（ログレベルによる制約付きの回避であること）を記載。
  - 注意: BLEスタックの再起動で復旧できない場合の最終手段として、ESP32本体の再起動
    （`App.safe_reboot()`）は残している。`connect_retry_limit` による再起動は行わない。
...
- Set the state to disconnected if an asynchronous connection cannot be started.
